...
object Main extends Helper {
  ...
  protected def configure = {
    val platformsFor71 = List("ubuntu", "windows", "aix5", "aix7")   // 7.1.x platforms 
    // FuseForge Projects
    forge_git("jansi")
    // forge_git("console")
    ...
    // ActiveMQ Branches
    activemq("trunk-fuse")
    activemq("5.7.0.fuse-7-1-x-stable") using { p => p.jdks("jdk7", "jdk6"); p.labels=platformsFor71; p.addBuild(p.dualjdk) }
    activemq("5.6.x-fuse")
    activemq("5.5.x-fuse")
